...
if (forge.is.mobile()) {
	...
	asyncTest("Add a button", 1, function() {
		forge.topbar.removeButtons(function () {
			...
			askQuestion("Press the star shaped button (with no number on iOS 7)", {
				"Nothing happened": function () {
					ok(false, "User claims failure");
					start();
				}
	...
	asyncTest("Tint", 1, function() {
		forge.topbar.setTint([0,100,0,255]);
		askQuestion("Is the bar now dark green?", {
			Yes: function () {
				ok(true, "Success");
				start();
			},
	...
	asyncTest("Add a button - tint", 1, function() {
		forge.topbar.addButton({text: "®é∂", tint: [100,0,0,255]}, runOnce(function () {
			ok(true, "Success");
			forge.topbar.removeButtons();
			start();
		}));
		askQuestion("Press the button if it shows '®é∂' and is dark red", {
			"Nothing happens or the button is not as described": function () {
				ok(false, "User claims failure");
				forge.topbar.removeButtons();
				start();
