...
+ (void)show:(ForgeTask*)task {
	if (hidden) {
		...
		UIEdgeInsets newInset = UIEdgeInsetsMake(inset.top + topbar.frame.size.height, inset.left, inset.bottom, inset.right);
		...
		[[[ForgeApp sharedApp] viewController].view bringSubviewToFront:topbar];
...
+ (void)hide:(ForgeTask*)task {
	if (!hidden) {
		...
		[[[ForgeApp sharedApp] viewController].view sendSubviewToBack:topbar];
